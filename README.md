...
validation.ipynb        # Полная валидация (этапы 1-4)
requirements.txt
...
- Дообучение на новых данных было решено не делать по двум причинам:
  1. В условии задания не определён целевой сценарий эксплуатации (язык, канал связи,
     характер реального шума). Без этого неясно, на каких данных и под какую конкретную
     задачу дообучать модель: дообучение вслепую рискует оптимизировать её не под тот
     сценарий, с которым она реально столкнётся.
  2. ECAPA-TDNN обучена на VoxCeleb1+2 — сотнях тысяч примеров и тысячах дикторов.
     Дообучение на несопоставимо малом количестве синтетически зашумлённых пар
     (сотни–тысячи) с высокой вероятностью приведёт к переобучению под конкретный
     синтетический шум, а не к реальному улучшению устойчивости.
